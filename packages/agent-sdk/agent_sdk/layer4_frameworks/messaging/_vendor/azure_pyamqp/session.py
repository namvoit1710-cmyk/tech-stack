...
from .constants import ConnectionState, Role, SessionState, SessionTransferState
...
class Session(object):  # pylint: disable=too-many-instance-attributes
    ...
    def _incoming_begin(self, frame):
        ...
        self.remote_properties = frame[
            7
        ]  # incoming map of properties about the session
    ...
    def _incoming_attach(self, frame):
        try:
            ...
            # Reject Link
            _LOGGER.debug(
                "Unable to attach new link: %r", e, extra=self.network_trace_params
            )
    ...
    def _wait_for_response(
        self, wait: Union[bool, float], end_state: SessionState
    ) -> None:
        if wait is True:
    ...
    def end(self, error: Optional[AMQPError] = None, wait: bool = False) -> None:
        try:
            ...
            _LOGGER.info(
                "An error occurred when ending the session: %r",
                exc,
                extra=self.network_trace_params,
            )
            self._set_state(SessionState.UNMAPPED)
