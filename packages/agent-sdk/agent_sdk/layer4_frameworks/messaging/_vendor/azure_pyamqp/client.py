...
from .message import Message, _MessageDelivery
            ...
            while self._connection and not self._shutdown:
                ...
                if elapsed_time >= self._keep_alive_interval:
                    self._connection.listen(
                        wait=self._socket_timeout, batch=self._link.current_link_credit
                    )
...
class SendClient(AMQPClient):
    ...
    def send_message(self, message, *, timeout: float = 0, **kwargs):
        ...
        self._do_retryable_operation(
            self._send_message_impl, message=message, timeout=timeout, **kwargs
        )
...
class ReceiveClient(AMQPClient):  # pylint:disable=too-many-instance-attributes
    ...
    def _client_run(self, **kwargs):
        ...
        try:
            ...
            _logger.info(
                "Timeout reached, closing receiver.", extra=self._network_trace_params
            )
            self._shutdown = True
            return False
        return True
