...
def encode_uint(
    ...
    try:
        ...
        raise ValueError(
            "Value supplied for unsigned int invalid: {}".format(value)
        ) from exc
...
def encode_ulong(
    ...
    try:
        ...
        raise ValueError(
            "Value supplied for unsigned long invalid: {}".format(value)
        ) from exc
...
def encode_decimal128(
    output: bytearray, value: Decimal, with_constructor: bool = True, **kwargs: Any
) -> None:
    """
...
def encode_described(
    output: bytearray, value: Tuple[Any, Any], _: bool = None, **kwargs: Any
) -> None:  # type: ignore
    ...
    for key, data in value.items():
        ...
        try:
            cast(List, fields[VALUE]).append(
                (field_key, {TYPE: data[TYPE], VALUE: data[VALUE]})
            )
        except (KeyError, TypeError):
