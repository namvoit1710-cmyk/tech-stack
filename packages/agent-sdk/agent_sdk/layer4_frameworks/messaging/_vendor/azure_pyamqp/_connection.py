...
from .error import AMQPConnectionError, AMQPError, ErrorCondition
from .performatives import CloseFrame, OpenFrame
...
def get_local_timeout(
    now: float, idle_timeout: float, last_frame_received_time: float
) -> bool:
...
class Connection:  # pylint:disable=too-many-instance-attributes
    ...
    def __init__(  # pylint:disable=too-many-locals
        ...
        **kwargs: Any,
        ...
        self._network_trace_params = {
            "amqpConnection": self._container_id,
            "amqpSession": "",
            "amqpLink": "",
        }
        ...
        if (
            self._transport_type.value == TransportType.Amqp.value
            and self._socket_timeout is None
        ):
            self._socket_timeout = SOCKET_TIMEOUT
        elif (
            self._transport_type.value == TransportType.AmqpOverWebsocket.value
            and self._socket_timeout is None
        ):
        ...
        if transport:
            ...
            sasl_transport: Union[Type[SASLTransport], Type[SASLWithWebSocket]] = (
                SASLTransport
            )
    ...
    def _send_frame(self, channel: int, frame: NamedTuple, **kwargs: Any) -> None:
        ...
        if self._can_write():
            ...
            _LOGGER.info(
                "Cannot write frame in current state: %r",
                self.state,
                extra=self._network_trace_params,
            )
    ...
    def _incoming_open(self, channel: int, frame) -> None:
        ...
        if self.state == ConnectionState.OPENED:
            _LOGGER.error(
                "OPEN frame received in the OPENED state.",
                extra=self._network_trace_params,
            )
            self.close()
        if frame[4]:
            self._remote_idle_timeout = cast(
                float, frame[4] / 1000
            )  # Convert to seconds
        ...
        if self.state == ConnectionState.OPEN_SENT:
            ...
            _LOGGER.error(
                "Connection is an illegal state: %r",
                self.state,
                extra=self._network_trace_params,
            )
    ...
    def _incoming_close(self, channel: int, frame: Tuple[Any, ...]) -> None:
        ...
        if frame[0]:
            ...
            _LOGGER.warning(
                "Connection closed with error: %r",
                frame[0],
                extra=self._network_trace_params,
            )
    ...
    def _process_outgoing_frame(self, channel: int, frame) -> None:
        ...
        if self.state not in [
            ...
            raise AMQPConnectionError(
                ErrorCondition.SocketError, description="Connection not open."
            )
        now = time.time()
    ...
    def _wait_for_response(
        self, wait: Union[bool, float], end_state: ConnectionState
    ) -> None:
    ...
    def listen(
        self, wait: Union[float, bool] = False, batch: int = 1, **kwargs: Any
    ) -> None:
    ...
    def close(self, error: Optional[AMQPError] = None, wait: bool = False) -> None:
        ...
        try:
            ...
            _LOGGER.info(
                "An error occurred when closing the connection: %r",
                exc,
                extra=self._network_trace_params,
            )
            self._set_state(ConnectionState.END)
