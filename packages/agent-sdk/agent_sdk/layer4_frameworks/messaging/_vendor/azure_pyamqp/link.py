...
class Link:  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.link_credit = (
            link_credit if link_credit is not None else DEFAULT_LINK_CREDIT
        )
    ...
    def _set_state(self, new_state: LinkState) -> None:
        ...
        try:
            ...
            _LOGGER.error(
                "Link state change callback failed: '%r'",
                e,
                extra=self.network_trace_params,
            )
    ...
    def _outgoing_attach(self) -> None:
        ...
        attach_frame = AttachFrame(
            ...
            if self.state == LinkState.ATTACH_RCVD
            else None,
    ...
    def _incoming_attach(self, frame) -> None:
        ...
        if self._on_attach:
            try:
                ...
                _LOGGER.warning(
                    "Callback for link attach raised error: %r",
                    e,
                    extra=self.network_trace_params,
                )
    ...
    def _outgoing_detach(
        self, close: bool = False, error: Optional[AMQPError] = None
    ) -> None:
    ...
    def _incoming_detach(self, frame) -> None:
        ...
        if self.state == LinkState.ATTACHED:
            ...
            and self.state in [LinkState.ATTACH_SENT, LinkState.ATTACH_RCVD]
        ):
        ...
        if frame[2]:  # error
            ...
            error_cls = (
                AMQPLinkRedirect
                if condition == ErrorCondition.LinkRedirect
                else AMQPLinkError
            )
            ...
            if self.state != LinkState.DETACH_SENT:
                ...
                self._error = AMQPLinkError(
                    ...
                    description="Link detached unexpectedly.",
                    retryable=True,
                )
            self._set_state(LinkState.DETACHED)
    ...
    def detach(self, close: bool = False, error: Optional[AMQPError] = None) -> None:
        ...
        try:
            ...
            _LOGGER.info(
                "An error occurred when detaching the link: %r",
                exc,
                extra=self.network_trace_params,
            )
            self._set_state(LinkState.DETACHED)
    ...
    def flow(self, *, link_credit: Optional[int] = None, **kwargs: Any) -> None:
        self.current_link_credit = (
            link_credit if link_credit is not None else self.link_credit
        )
