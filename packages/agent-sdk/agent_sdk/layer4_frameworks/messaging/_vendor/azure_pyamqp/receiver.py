...
from .performatives import DispositionFrame, TransferFrame
...
class ReceiverLink(Link):
    ...
    def _process_incoming_message(self, frame, message):
        try:
            ...
            _LOGGER.error(
                "Transfer callback function failed with error: %r",
                e,
    ...
    def _incoming_attach(self, frame):
        ...
        if frame[9] is None:  # initial_delivery_count
            _LOGGER.info(
                "Cannot get initial-delivery-count. Detaching link",
                extra=self.network_trace_params,
            )
    ...
    def _outgoing_disposition(
        ...
        if delivery_tag not in self._received_delivery_tags:
            raise AMQPException(
                ...
                description="Delivery tag not found.",
            )
    ...
    def send_disposition(
        ...
        *,
        ...
        delivery_state: Optional[
            Union[Received, Accepted, Rejected, Released, Modified]
        ] = None,
